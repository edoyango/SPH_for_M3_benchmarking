module ORB_sr_m
	...
	use globvar_para,	only: procid,numprocs,PhysPackSend,bounds_glob,n_process_neighbour,proc_neighbour_list,nphys_send,nphys_recv,&
        parttype,ierr
	use mpi
	use param,			only: dim,hsml
...
contains
	...
	! diffusion is considered
		...
		real(8):: xmin_rem,ymin_rem,xmax_rem,ymax_rem,xi,yi,dr,dr_min
		...
		do i = searchrange(1),searchrange(2)
			do d = 1,dim
				if ( (parts(i)%x(d).lt.bounds_glob(d,procid+1)) .or. (parts(i)%x(d).ge.bounds_glob(dim+d,procid+1))) then
		...
		if (nphys_send_all.gt.0) then
			loop_through_parts: do j = 1,nphys_send_all
				i = removal_list(j)
				xi = parts(i)%x(1); yi = parts(i)%x(2) !placeholder variables of particle position for code readibility
				do n = 1,n_process_neighbour
					pid = proc_neighbour_list(n) + 1
					xmin_rem = bounds_glob(1,pid); xmax_rem = bounds_glob(3,pid) !min-max x boundary for remote process, pid
					ymin_rem = bounds_glob(2,pid); ymax_rem = bounds_glob(4,pid) !min-max y boundary for remote process, pid
					if ( (xi.ge.xmin_rem) .and. (yi.ge.ymin_rem) .and. (xi.lt.xmax_rem) .and. (yi.lt.ymax_rem) ) then
				...
				do n = 1,n_process_neighbour
					...
					dr = 0.d0
					do d = 1,dim
						dr = dr + MAX(0.d0,bounds_glob(d,pid)-parts(i)%x(d),parts(i)%x(d)-bounds_glob(dim+d,pid))**2
					end do
					dr = SQRT(dr)
	...
	subroutine ORB_sendrecv_halo( request_in,request_out,nphys_recv_all,nrequest )
		...
		real(8):: xmin_rem,ymin_rem,xmax_rem,ymax_rem,xi,yi,t1,t2,xmin_loc,ymin_loc,xmax_loc,ymax_loc,dr
		...
		! Begin search
		xmin_loc = bounds_glob(1,procid+1) + scale_k*hsml; xmax_loc = bounds_glob(3,procid+1) - scale_k*hsml
		ymin_loc = bounds_glob(2,procid+1) + scale_k*hsml; ymax_loc = bounds_glob(4,procid+1) - scale_k*hsml
	1 	do i = pos0,pos1
			xi = parts(i)%x(1); yi = parts(i)%x(2)
			if ( (xi.le.xmin_loc) .or. (yi.le.ymin_loc) .or. (xi.ge.xmax_loc) .or.(yi.ge.ymax_loc) ) then
				do j = 1,n_process_neighbour
					pid = proc_neighbour_list(j) + 1
					xmin_rem = bounds_glob(1,pid) - scale_k*hsml; xmax_rem = bounds_glob(3,pid) + scale_k*hsml !min_max in x
					ymin_rem = bounds_glob(2,pid) - scale_k*hsml; ymax_rem = bounds_glob(4,pid) + scale_k*hsml !min-max in y
					if ( (xi.ge.xmin_rem) .and. (yi.ge.ymin_rem) .and. (xi.le.xmax_rem) .and. (yi.le.ymax_rem) ) then
