...
contains
	...
	! required
		...
		use param, 				only: dim,rh0,c,gamma,f
		...
		codrhodt(1:ntotal_loc) = 0d0
		indvxdt(:,1:ntotal_loc) = 0d0
		ardvxdt(:,1:ntotal_loc) = 0d0
		exdvxdt(1:dim-1,1:ntotal_loc) = 0d0
		exdvxdt(dim,1:ntotal_loc) = -g
		parts(1:ntotal_loc+nhalo_loc)%p = rh0*c**2*((parts(1:ntotal_loc+nhalo_loc)%rho/rh0)**gamma-1d0)/gamma
